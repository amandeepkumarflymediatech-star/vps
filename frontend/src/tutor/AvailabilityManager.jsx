...
const generateTimeSlots = () => {
  ...
  for (let hour = 9; hour <= 21; hour++) {
    ...
    let label = hour < 12 ? `${hour}:00 AM` : hour === 12 ? `12:00 PM` : `${hour - 12}:00 PM`;
...
const AvailabilityManager = () => {
  ...
  useEffect(() => { fetchAvailabilityForDate(selectedDate); }, []);
  useEffect(() => { if (!availability[selectedDate]) fetchAvailabilityForDate(selectedDate); }, [selectedDate]);
  ...
  const handleSave = async () => {
    try {
      ...
      const payload = timeSlots.map((slot) => {
        const state = availability[selectedDate]?.[slot.startTime] || { isAvailable: false, isBooked: false };
        return { ...slot, isAvailable: state.isAvailable, isBooked: state.isBooked };
      });
      await saveTutorAvailability({ date: selectedDate, availability: payload });
      setSaved(true);
  ...
  const countAvailable = Object.values(availability[selectedDate] || {}).filter(s => s?.isAvailable && !s?.isBooked).length;
      ...
      <div className="max-w-6xl mx-auto">
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-8">
          <div>
            ...
            <p className="text-slate-500 font-bold mt-1">Set your working hours for the week</p>
          </div>
          ...
          <button
            ...
            className={`w-full md:w-auto flex items-center justify-center gap-2 px-8 py-4 rounded-2xl font-bold transition-all active:scale-95 shadow-lg ${
              saved ? "bg-emerald-500 text-white shadow-emerald-200" : "bg-indigo-600 text-white hover:bg-indigo-700 shadow-indigo-200"
            }`}
          >
            {saving ? <Loader2 className="w-5 h-5 animate-spin" /> : saved ? <CheckCircle2 size={20} /> : <Save size={20} />}
            {saving ? "Updating..." : saved ? "Changes Saved" : "Save Availability"}
          </button>
        </div>
              ...
              return (
                ...
                >
                  <div className="text-center lg:text-left">
                    <p className={`text-[10px] uppercase font-black tracking-widest ${isActive ? "text-indigo-600" : "text-slate-400"}`}>
                      {d.day}
                    </p>
                    <p className={`text-xl font-bold ${isActive ? "text-slate-900" : "text-slate-500"}`}>
                      {d.dateNum} {d.month}
                    </p>
                  </div>
                  <ChevronRight size={20} className={`hidden lg:block transition-transform ${isActive ? "translate-x-1 text-indigo-600" : "opacity-0"}`} />
                </button>
              );
            })}
          </div>
          ...
          <div className="lg:col-span-9">
            <div className="bg-white rounded-[2.5rem] border border-slate-100 shadow-xl p-6 md:p-10 relative overflow-hidden">
              ...
              <div className="flex justify-between items-center mb-8 relative z-10">
                <div>
                  ...
                  <p className="text-slate-400 text-sm font-medium">{countAvailable} slots selected for this day</p>
                </div>
              </div>
                    ...
                    return (
                      ...
                      >
                        <div className="flex flex-col text-left">
                          <span className={`text-xs font-black uppercase tracking-tighter ${isAvailable ? "text-indigo-100" : "text-slate-400"}`}>
                            Session
                          </span>
                          <span className="font-bold text-sm md:text-base">{slot.label}</span>
                        </div>
              ...
              {!loading && timeSlots.length === 0 && (
                <div className="flex flex-col items-center justify-center py-20 text-slate-400">
                  ...
                  <p className="mt-4 font-medium">No slots available for this time range.</p>
                </div>
              )}
            </div>
            ...
            <div className="mt-6 flex flex-col sm:flex-row items-center justify-between bg-indigo-900 text-white p-6 rounded-[2rem] shadow-2xl">
              <div className="flex items-center gap-4 mb-4 sm:mb-0">
                ...
                <div>
                  <p className="text-indigo-200 text-xs font-black uppercase tracking-widest">Selected Date</p>
              ...
              <div className="text-center sm:text-right">
                ...
                <p className="text-indigo-300 text-[10px] font-black uppercase tracking-[0.2em]">Total Slots Active</p>
              </div>
            </div>
          </div>
        </div>
