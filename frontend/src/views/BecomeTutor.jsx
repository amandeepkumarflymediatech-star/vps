...
import toast, { Toaster } from "react-hot-toast"; // 1. Added Toaster
import { CheckCircle, Globe, DollarSign, Clock, BookOpen, Star } from "lucide-react";
import { applyTutorApi } from "../api/tutorApi";
import tutorHeroImg from "../assets/tutor-teaching.jpg";
...
const BecomeTutor = () => {
  ...
  const handleChange = (e) => {
    ...
    if (name === "name") {
      if (/[0-9]/.test(value)) {
        toast.error("Numbers are not allowed in names", { id: "name-err" });
        return;
      }
    }

    // 2. Phone Validation: Numbers only, limit 16
    if (name === "phone") {
      const numericValue = value.replace(/[^0-9+]/g, ""); // Allow only digits and +
      if (numericValue.length > 16) return;
      setForm({ ...form, [name]: numericValue });
      return;
    }

    // 3. Expertise Validation: Max 100 characters
    if (name === "expertise") {
      if (value.length > 100) {
        toast.error("Expertise cannot exceed 100 characters", { id: "exp-err" });
        return;
      }
    }
  ...
  const handleSubmit = async (e) => {
    ...
    // 4. Email Validation: must have @ and end with .com
    ...
    <div className="w-full bg-white font-sans text-slate-900">
            ...
            <div className="relative min-h-[480px]">
              {!submitted ? (
                <div className="animate-in fade-in slide-in-from-bottom-4 duration-700">
                  <h3 className="text-3xl font-bold mb-2">Apply for Tutor Position</h3>
                ...
                <div className="flex flex-col items-center justify-center text-center h-[480px] animate-in zoom-in duration-500">
                  ...
                  <h3 className="text-3xl font-bold text-slate-900 mb-4 uppercase tracking-tight">Application Received!</h3>
                  <p className="text-slate-600 leading-relaxed">
                    Thank you for your interest in <strong>The English Raj</strong>. <br />
                    Our HR team will review your profile and send an update to your email within 48 business hours.
                  </p>
                </div>
              )}
            </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-10">
            {[
              ...
              <div key={i} className="bg-white p-8 rounded-2xl shadow-sm border border-slate-100 hover:shadow-md transition-shadow">
                ...
                <h4 className="text-lg font-black mb-3 tracking-wide">{perk.title}</h4>
              ...
              {[
                ...
                <div key={i} className="flex gap-5 md:gap-8 group">
                  ...
                  <div className="border-l-2 border-slate-100 pl-6 group-hover:border-[#0852A1] transition-colors">
                    <h4 className="text-lg md:text-xl font-black text-slate-800 mb-1 tracking-tight">{item.title}</h4>
      ...
      <section className="py-20 bg-gradient-to-r from-[#0B3C66] to-[#0852A1] text-center text-white">
        <div className="max-w-4xl mx-auto px-6 text-white">
          <h2 className="text-3xl md:text-5xl font-bold mb-8 text-white">Ready to define the future of English education?</h2>
          <button
            onClick={() => window.scrollTo({ top: 0, behavior: 'smooth' })}
            className="bg-white text-[#0852A1] px-10 py-4 rounded-full font-bold text-lg hover:bg-yellow-400 hover:text-slate-900 transition-all shadow-xl"
          >
            Apply Now
          </button>
        </div>
