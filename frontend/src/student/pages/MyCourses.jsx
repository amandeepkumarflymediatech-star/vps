...
import { useRouter } from "next/navigation";
import Link from "next/link";
import { Calendar, ChevronLeft, ChevronRight, Clock, LayoutGrid } from "lucide-react";
import { getTutors } from "@/api/tutorApi";
import { getStudentClasses } from "@/api/student.api";
      ...
      try {
        ...
        const tutorsWithAvailability = await Promise.all(
          rawTutors.map(async (t) => {
            ...
            try {
              const classesRes = await getStudentClasses({ tutorId: id });
              const firstClass = classesRes.data?.data?.[0]?.schedule?.[0];
              if (firstClass) {
                nextAvailability = `${firstClass.day}, ${firstClass.startTime}`;
              }
            } catch { }
        ...
        <div className="flex flex-wrap bg-gray-100 p-1.5 rounded-2xl">
          {TABS.map((tab) => (
            <button
              key={tab}
              onClick={() => setActiveTab(tab)}
              className={`px-5 py-2 rounded-xl text-sm font-bold transition ${activeTab === tab ? "bg-white text-[#6335F8] shadow" : "text-gray-500"
                }`}
            >
              {tab}
            </button>
      ...
      <div>
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {paginatedTutors.map((tutor, i) => {
            // Assign lesson & amount dynamically
            const lessonOptions = Object.keys(LESSON_PACKAGES);
            const defaultLesson = lessonOptions[i % lessonOptions.length];
            const amount = LESSON_PACKAGES[defaultLesson];

            return (
              <div key={tutor.id} className="bg-white p-6 rounded-3xl border hover:shadow-xl transition">
                <div className="flex gap-4 mb-4">
                  <img src={tutor.img} className="w-16 h-16 rounded-2xl" alt="" />
                  <div>
                    <h4 className="font-black">{tutor.name}</h4>
                    <p className="text-xs text-gray-400 font-bold">{tutor.sessions}+ Sessions</p>
                  </div>
                </div>
                ...
                {/* <button
                  onClick={() =>
                    router.push(
                      `/payment/upi?tutorId=${tutor.id}&lessons=${encodeURIComponent(defaultLesson)}&amount=${amount}`
                    )
                  }
                  className="w-full py-3 rounded-xl bg-purple-50 text-[#6335F8] font-black hover:bg-[#6335F8] hover:text-white transition"
                >
                ...
                <button
                  onClick={() =>
                    router.push(
                      `/CoursesPricing?tutorId=${tutor.id}&lessons=${encodeURIComponent(defaultLesson)}&amount=${amount}`
                    )
                  }
                  className="w-full py-3 rounded-xl bg-purple-50 text-[#6335F8] font-black hover:bg-[#6335F8] hover:text-white transition"
                >
